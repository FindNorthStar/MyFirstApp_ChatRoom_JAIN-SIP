...
public class UserAdapter extends ArrayAdapter<MyUser> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        {//online
            viewHolder.userImage.setImageResource(user.imageGroup_on[user.getImageID()]);
        }
        else
        {
            viewHolder.userImage.setImageResource(user.imageGroup_off[user.getImageID()]);
        }
        int n=user.getCurrentStatus();
        viewHolder.statusImage.setImageResource(user.imageSatus[n]);
